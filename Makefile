...
help:
	...
	@echo "  make storage-test-placement Valida o caminho físico do PVC de teste atual"
	...
	@echo "  make observability-grafana-dashboard-audit Audita dashboards Grafana e referências de datasource"
	@echo "  make otel-go-demo-install  Builda, importa no K3s e publica o demo Go instrumentado"
	@echo "  make otel-go-demo-test     Gera trace distribuído e confirma os dois serviços no Tempo"
	@echo "  make otel-go-demo-metrics-test Gera tráfego e valida métricas customizadas no Prometheus"
	@echo "  make otel-go-demo-incident-test Injeta falha temporária no downstream e valida métricas/alerta/log/trace/recuperação"
	@echo "  make otel-go-demo-status   Mostra Deployment, Pod, Service e ServiceMonitor do demo Go"
	@echo "  make otel-go-demo-delete   Remove o demo Go do namespace lab"
	@echo "  make firecrawl-migration-audit Audita o Docker Compose Firecrawl sem imprimir secrets"
	@echo "  make firecrawl-k8s-validate Valida o scaffold Kubernetes Firecrawl sem aplicar recursos"
	@echo "  make firecrawl-k8s-status  Mostra recursos Firecrawl caso o namespace já exista"
...
storage-test-placement:
	@PV=$$(kubectl get pvc persistence-test -n lab -o jsonpath='{.spec.volumeName}'); \
	  test -n "$$PV" || { echo "ERROR: persistence-test PVC is not bound" >&2; exit 1; }; \
	  PATH_VALUE=$$(kubectl get pv "$$PV" -o json | jq -r '.spec.hostPath.path // .spec.local.path // empty'); \
	  echo "Provisioned $$PV at $$PATH_VALUE"; \
	  test -n "$$PATH_VALUE" || { echo "ERROR: PV has neither spec.hostPath.path nor spec.local.path" >&2; kubectl get pv "$$PV" -o yaml; exit 1; }; \
	  case "$$PATH_VALUE" in \
...
observability-logging-status:
	@helm list -n monitoring | grep -E '^(loki|alloy|NAME)' || true
	@echo
	kubectl get pods -n monitoring -o wide | grep -E 'loki|alloy|NAME' || true
	@echo
	kubectl get services -n monitoring | grep -E 'loki|alloy|NAME' || true
	@echo
	kubectl get pvc -n monitoring -o wide | grep -E 'loki|NAME' || true
...
observability-grafana-dashboard-audit:
	bash scripts/grafana-dashboard-audit.sh
...
otel-go-demo-metrics-test:
	bash scripts/otel-go-demo.sh metrics-test

otel-go-demo-incident-test:
	bash scripts/otel-go-demo-incident.sh

otel-go-demo-delete:
	bash scripts/otel-go-demo.sh delete

firecrawl-migration-audit:
	bash scripts/firecrawl-migration-audit.sh

firecrawl-k8s-validate:
	bash scripts/firecrawl-k8s.sh validate

firecrawl-k8s-status:
