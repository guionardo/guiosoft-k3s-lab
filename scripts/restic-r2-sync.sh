...
BACKUP_DIR_CANON="$(readlink -f "$BACKUP_DIR")"
[[ -d "$BACKUP_DIR_CANON" ]] || { echo "error: backup directory not found: $BACKUP_DIR" >&2; exit 1; }
if [[ -n "$EXPLICIT_ARCHIVE" ]]; then
  ...
  case "$ARCHIVE" in "$BACKUP_DIR_CANON"/*) ;; *) echo "error: explicit archive must be below $BACKUP_DIR_CANON" >&2; exit 1;; esac
else
  ARCHIVE="$(find "$BACKUP_DIR_CANON" -maxdepth 1 -type f -name 'k3s-*.tar.gz' -printf '%T@ %p\n' | sort -nr | head -n1 | cut -d' ' -f2-)"
fi
...
(cd "$BACKUP_DIR_CANON"; sha256sum -c "$(basename "$CHECKSUM")")
