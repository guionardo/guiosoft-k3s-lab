...
command -v nft >/dev/null || { echo "error: nft not found; DR WAN isolation cannot be verified" >&2; exit 1; }
if ! nft list table inet "$ISOLATION_TABLE" >/dev/null 2>&1; then
  echo "error: DR WAN isolation is not active (missing inet/$ISOLATION_TABLE)" >&2
  echo "enable it first with: sudo $REPO_ROOT/scripts/dr-network-isolation.sh enable" >&2
  exit 1
fi
...
# Bootstrap files from the clean DR datastore must not be mixed with the
# restored datastore. Preserve them for rollback and let K3s reconstruct them.
