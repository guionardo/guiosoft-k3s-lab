...
scripts=(dr-target-init.sh dr-target-reset.sh dr-network-isolation.sh dr-preflight.sh dr-rehearsal-start.sh dr-rehearsal-status.sh dr-rehearsal-report.sh dr-restore-k3s.sh k3s-backup-verify.sh dr-neutralize.sh dr-pv-import.sh dr-pv-remap.sh dr-oci-preload.sh dr-activate-observability.sh dr-kit-verify.sh dr-bundle-verify.sh dr-recover.sh dr-recovery-set-export.sh dr-recovery-set-materialize.sh dr-recovery-set-materialize-verify.sh dr-recovery-bundle-build.sh)
...
Contains the complete isolated recovery toolchain, formal pre-bootstrap rehearsal timer, bounded-consistency recovery-set export/materialization/verifier, portable bundle builder/verifier, checkpointed recovery orchestrator, guarded activation helper and reproducible rehearsal reporter. It intentionally contains no plaintext credentials, private age identity, K3s server token, Restic password, or R2 secret keys. Verify SHA256SUMS before use. GitHub access is not required after materialization.
EOF
